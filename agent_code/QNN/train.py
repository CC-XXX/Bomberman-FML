...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if len(events) != 0:
        ...
        current_reward[0][self.last_actions[-1]] = reward_from_events(self, events, new_game_state['self'][3])  # r
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.last_pos = last_game_state['self'][3]

    current_reward = np.zeros((1, n_actions))

    current_reward[0][self.last_actions[-1]] = reward_from_events(self, events, last_game_state['self'][3])
    self.rewards = np.vstack((self.rewards, current_reward))
...
def reward_from_events(self, events: List[str], last_game_state_pos) -> int:
    """
    *This is to modify the rewards your agent get so as to en/discourage
    ...
    for event in events:
        ...
        # valid action for a walk
            ...
            reward_sum += 100
        elif len(events)==1 and self.last_pos == last_game_state_pos and event != e.WAITED and event != e.BOMB_DROPPED:
            reward_sum += -5 # invalid action
